...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # we will be using iterative computation
    score = 0
    while num_rolls > 0:
        roll = dice()
        # pig out
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    #free bacon
    if num_rolls == 0:
        digits = [int(d) for d in str(opponent_score)]
        score = max(digits) + 1
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while max(score, opponent_score) < 100:
        ...
        if who == 0:
            points = take_turn(strategy0(score, opponent_score), opponent_score, dice)
            score = score + points
        else:
            points = take_turn(strategy1(opponent_score, score), score, dice)
            opponent_score = opponent_score + points
        who = other(who)
        # swine swap
        if (score * 2) == opponent_score or (opponent_score * 2) == score:
            score, opponent_score = opponent_score, score
        # print('score0 = ' + str(score) + ' score1 = ' + str(opponent_score))
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    number_of_dice = 1
    scores = []
    while number_of_dice < 11:
        average = make_averaged(roll_dice)(number_of_dice)
        scores.append(average)
        print(str(number_of_dice) + ' dice scores ' + str(average) + ' on average')
        number_of_dice = number_of_dice + 1
    max_score = max(scores)
    num_dice = scores.index(max_score) + 1
    return num_dice
...
def run_experiments():
    ...
    if True: # Change to True to test always_roll(8)
        print('always_roll(5) win rate:', average_win_rate(always_roll(8)))
...
def bacon_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    #bacon margin = 8. baseline num rolls = 5
    digits_opponent_score = [int(d) for d in str(opponent_score)]
    if (max(digits_opponent_score) + 1 >= BACON_MARGIN):
        return 0
    else:
...
def swap_strategy(score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    digits_opponent_score = [int(d) for d in str(opponent_score)]
    digits_score = [int(d) for d in str(score)]
    beneficial_swap_score = 1 + max(digits_opponent_score)
    beneficial_swap_opponent_score = 1 + max(digits_score)
    if (opponent_score / 2 == score + beneficial_swap_score) or (score / 2 == opponent_score + beneficial_swap_opponent_score):
        return 0
    elif ((score + beneficial_swap_score) / 2 == opponent_score) or ((opponent_score + beneficial_swap_opponent_score) / 2 == score):
        return BASELINE_NUM_ROLLS
    elif (max(digits_opponent_score) + 1 >= BACON_MARGIN) or (max(digits_score) + 1 >= BACON_MARGIN):
        return 0
    else:
        return BASELINE_NUM_ROLLS
