...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < 100 and opponent_score < 100:
        if who == 0:
            ...
            print("Error with turn allocation.")
            score, opponent_score = 100, 100
        #Swine swap
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    while k < 10:
        k = k + 1
        result = make_averaged(roll_dice)(k, dice)
        print(k, 'dice scores', result, 'on average')
        if result > highest: #keeping track of how many dice produce the higest value
            highest = result
            highest_dice = k
    return highest_dice #dice with the highest average value
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    #Covering whether to swap if beneficial or avoid swapping if harmful
    if opponent_score == 2 * (score + 1 + max(opponent_score // 10, opponent_score % 10)):
        return 0
    if opponent_score == 0.5 * (score + 1 + max(opponent_score // 10, opponent_score % 10)):
        return BASELINE_NUM_ROLLS
    else:  #when rolling a 0 does not cause a swap
        if 1 + max(opponent_score // 10, opponent_score % 10) >= BACON_MARGIN:
            return 0
        else:
            return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if opponent_score == 2 * (score + 1 + max(opponent_score // 10, opponent_score % 10)):
        return 0
    #Do free bacon if it can force your opponent to roll 4-sided dice
    elif (opponent_score + score + 1 + max(opponent_score // 10, opponent_score % 10)) % 7 == 0:
        return 0
    elif opponent_score == 0.5 * (score + 1 + max(opponent_score // 10, opponent_score % 10)):
        return BASELINE_NUM_ROLLS
    #if currently rolling 4-sided dice
    # elif (score + opponent_score) % 7 == 0:
        # # def max_scoring_num_rolls_np(dice=six_sided):
            # # k = 0
            # # highest = 0
            # # highest_dice = 0
            # # while k < 10:
                ...
                # # result = make_averaged(roll_dice)(k, dice)
                # # #print(k, 'dice scores', result, 'on average')
                # # if result > highest: #keeping track of how many dice produce the highest value
                    # # highest = result
                    # # highest_dice = k
            # # return highest_dice #dice with the highest average value
        # # return max_scoring_num_rolls_np(four_sided)
        # return 4
    #when no other strategies give advantage, follow bacon strategy
    else:  
        if 1 + max(opponent_score // 10, opponent_score % 10) >= BACON_MARGIN:
            return 0
        else:
